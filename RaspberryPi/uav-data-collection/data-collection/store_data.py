...
if __name__ == '__main__':
    ...
    filename = f"../data/sensor_data_{initial_timestamp}.csv"

    with open(filename, 'w', newline='') as csvfile:
        fieldnames = ["timestamp", "uv_data"]
        ...
        try:
            while True:
                ...
                writer.writerow({'timestamp': timestamp, 'uv_data': uv_val})
                print(f"UV value: {uv_val}   at {timestamp}")
                csvfile.flush()  # Ensure data is written to the file
                time.sleep(10)  # Record data every 10 seconds
